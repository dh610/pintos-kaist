...
process_fork (const char *name, struct intr_frame *if_ UNUSED) {
	...
	curr->sysf = if_;
	tid_t res = thread_create (name, PRI_DEFAULT, __do_fork, curr);
...
__do_fork (void *aux) {
	...
	struct intr_frame *parent_if = parent->sysf;
	bool succ = true;
	...
	for (int i = 2; i <= parent->fd_end; i++) {
		fp = parent->fd_table[i];
		if (fp != NULL)
			current->fd_table[i] = file_duplicate(fp);
	}
...
process_wait (tid_t child_tid UNUSED) {
	...
	//can't find child (already died or other's child)
	if (child == NULL) return -1;

	//wait for one child can call once
	if (!sema_try_down(&child->wait_cnt)) return -1;
	
	list_remove(&child->child_elem);
	sema_up(&child->exiting);
...
setup_stack (struct intr_frame *if_) {
	...
	if (vm_alloc_page(VM_ANON | VM_MARKER_0, stack_bottom, true)) {
		...
		if_->rsp = USER_STACK;
		thread_current()->spp = stack_bottom;
	}
